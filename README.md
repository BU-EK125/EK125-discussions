...
This site is a public, permanent companion to the main [EK125 site](https://github.com/BU-EK125/EK125) -- it stays up and accessible indefinitely, not just for the current semester. It collects discussion-section notebooks: worked examples, live coding walkthroughs, and review material (exam/quiz review included -- unlike the main course site, that's welcome here).
...
**Want a cell to intentionally raise an error** (e.g. demonstrating that tuples are immutable, or a common type error)? Tag that cell `raises-exception`, or the build will treat the error as broken code and fail. In Jupyter/JupyterLab: View → Cell Toolbar → Tags, then type `raises-exception` into the tag box for that cell.
...
**To add or modify a discussion notebook:**

1. Create or edit the file at `discussions/WeekN.ipynb`. Just write real, working code -- there's nothing extra to do before saving; the site runs it for you at build time.
...
## https://BU-EK125.github.io/EK125-discussions/intro.html
