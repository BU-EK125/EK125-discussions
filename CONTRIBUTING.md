...
This guide walks through the whole process of editing a discussion notebook and getting your change published, assuming you've never used git or GitHub before. Follow the steps in order the first few times -- once it clicks, it takes about a minute of actual work per change.
...
3. **Ask to be added as a collaborator** on `BU-EK125/EK125-discussions` (whoever manages the repo can add your GitHub username under the repo's Settings → Collaborators). Without this, you won't be able to push changes.
4. **Download ("clone") the repo to your computer.** Open Terminal (or Git Bash on Windows), navigate to wherever you'd like the folder to live (e.g., `cd Documents`), then run:
   ```
   git clone https://github.com/BU-EK125/EK125-discussions.git
   ```
   This creates a folder called `EK125-discussions` with a full copy of the repo. You only do this once -- from now on you'll just update this same folder.
...
```
git checkout -b add-week10-discussion
```
(Replace `add-week10-discussion` with whatever describes your change.)

### 3. Make your edit

Open the file in whatever editor you like (VS Code, Jupyter, Colab) and make your change. Notebooks live in `discussions/WeekN.ipynb`. If you're adding a brand-new page rather than editing an existing one, you'll also need to add one line to `_toc.yml` (see the [README](README.md)).
...
```
git add .
git commit -m "Add Week 10 discussion notebook"
git push -u origin add-week10-discussion
```
...
- `git push ...` uploads your branch to GitHub. The `-u origin add-week10-discussion` part is only needed the very first time you push this branch; after that, plain `git push` works.
...
```
remote: Create a pull request for 'add-week10-discussion' on GitHub by visiting:
remote:      https://github.com/BU-EK125/EK125-discussions/pull/new/add-week10-discussion
```
